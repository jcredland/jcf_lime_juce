...
class ApplicationActivtyMonitor : public Timer, MouseListener
{
public:
    /**
    ...
    ~ApplicationActivtyMonitor() { Desktop::getInstance().removeGlobalMouseListener (this); }

    void onApplicationBecomesActive (std::function<void()> fun) { applicationNowActiveCallback = fun; }

    bool isApplicationRecentlyActive() const { return isActive; }
...
{
public:
    ArraySaver (const File& saveFile, const Identifier& identifier) : identifier (identifier), saveFile (saveFile) {}
...
public:
    ...
    void setRateLimit (int milliseconds) { rateLimitMilliSeconds = milliseconds; }
...
/**
 * A function that vertically centres a vector of components maintaining their size and spacing within a parent component.
 */
inline void centreComponentsVertically (std::vector<Component*> components, const Rectangle<int>& withInArea)
{
    if (components.empty())
        return;
